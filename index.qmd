...
for H in HORIZONS:
    ...
    X_tr, y_tr, X_val, y_val, X_te, y_te = split_scale_window(base, feat_cols, H)

    model = build_lstm_classifier(LOOKBACK, len(feat_cols))
    ...
    tau, _ = tune_threshold(y_val, y_val_prob, metric="balacc")

    # Scores on test
    s = score_table(y_te, y_te_prob, tau)
    s["H"] = H
    all_scores.append(s)
    print(f"Test metrics (H={H}):", {k: (round(v,4) if isinstance(v,float) else v) for k,v in s.items()})

    # Plots
    plot_learning_curves(history, title=f"H={H}")
    plot_roc_pr(y_val, y_val_prob, title=f"H={H} (validation)")
    plot_roc_pr(y_te,  y_te_prob,   title=f"H={H} (test)")
    plot_confusion_at_tau(y_te, y_te_prob, tau, title=f"H={H} (test)")
    plot_prob_hist(y_te_prob, title=f"H={H} (test)")

# --- Summary table
scores_df = pd.DataFrame(all_scores)[["H","Acc","BalAcc","F1","ROC_AUC","PR_AUC","IC","tau"]]
try:
    from IPython.display import display
    display(scores_df.sort_values("H").reset_index(drop=True))
except Exception:
    print(scores_df.sort_values("H").reset_index(drop=True))
