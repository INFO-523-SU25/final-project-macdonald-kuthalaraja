...
## Q2 \| Step 2 — Factor Features (2014–2024)
```{python}
...
## Q2 \| Step 4 — Hierarchical Clustering (Ward)
```{python}
#| label: q2-step4-linkage
#| fig-width: 18
#| fig-height: 7
#| fig-align: center
#| dpi: 150
...
# Feature means per cluster (z-scored back using scaler params)
centers = (pd.DataFrame(Xz, index=feat.index, columns=['Momentum12m','Vol20_Ann','Size_logDV'])
           .assign(Cluster=feat['Cluster'])
           .groupby('Cluster').mean())

fig, ax = plt.subplots(figsize=(18,6.5))
im = ax.imshow(centers.values, aspect='auto')
ax.set_xticks(range(centers.shape[1])); ax.set_xticklabels(centers.columns)
ax.set_yticks(range(centers.shape[0])); ax.set_yticklabels([f"C{c}" for c in centers.index])
ax.set_title("Cluster Profiles (feature z-scores)")
for i in range(centers.shape[0]):
    for j in range(centers.shape[1]):
        ax.text(j, i, f"{centers.values[i,j]:.2f}", ha='center', va='center', fontsize=9, color='white' if abs(centers.values[i,j])>0.6 else 'black')
...
ax = mix.plot(kind='bar', stacked=True, figsize=(18,7), width=0.85)
ax.set_title("Sector Mix within Clusters (%)")
ax.set_xlabel("Cluster"); ax.set_ylabel("Percent of names")
ax.legend(bbox_to_anchor=(0.5, -0.2), loc="upper center", ncol=5, frameon=False, fontsize=9, title="Sector")
plt.tight_layout(rect=[0,0.08,1,1]); plt.show()
```

## Q2 | Step 6 — Validation (Silhouette & Bootstrap Stability)
```{python}
#| label: q2-step6-validate
#| fig-width: 16
#| fig-height: 6
#| fig-align: center
#| dpi: 150
...
def cluster_labels(Xz, k, seed):
    Lb = linkage(Xz, method='ward')
...
lab1 = cluster_labels(Xz[idx1], k, 1)
lab2 = cluster_labels(Xz[idx2], k, 2)
...
**Goal:** Organize S&P 500 stocks into a taxonomy based on persistent risk/return exposures.  
**Features:** 12-month momentum, 20-day realized volatility, log dollar-volume (size proxy).  
**Method:** Build factor matrix (2014–2024 averages) → standardize → hierarchical clustering (Ward).  
**Validation:** Silhouette ≈ _X.XXX_; bootstrap ARI ≈ _X.XXX_.  
**Use:** Natural peer groups for portfolio construction, risk diversification, and rotation ideas.
