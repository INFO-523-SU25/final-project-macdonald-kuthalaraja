...
- Realized volatility (20d rolling std of returns), intraday range  
...
::: tiny
**Feature signals**
- 20-day realized volatility and **VIX** are the strongest drivers.
- **RSI** and **volume ratios** add incremental, smaller lift.

**Model performance**
- Random Forest reached **balanced accuracy / macro-F1 ≈ 0.6–0.7** (holdout).
- Most errors are in the **Medium** class (overlaps Low/High).

**Market interpretation**
- **Low** and **High** regimes are easier to separate (calm vs. turbulence).
- **Medium** reflects transition phases where signals are noisy.

**Practical lesson**
- Regime classification is useful for **risk flagging** (position sizing, hedging).
- The “middle ground” remains hardest to predict—consistent with trading intuition.
:::
...
**Method:**  
- Construct factor-exposure matrix for each stock **(2014–2024 averages)**
...
tickers_yahoo = [swap_map.get(t, t) for t in tickers]

# Download OHLCV data (2014–2024)
data = yf.download(
    tickers_yahoo,
...
# Count companies by Sector + Sub-Industry
sector_industry_counts = (
    sp500.groupby(["GICS Sector", "GICS Sub-Industry"])["Symbol"]
    .count()
    .unstack(fill_value=0)
)

# Plot stacked bar (1.5x bigger)
ax = sector_industry_counts.plot(
    kind="barh", stacked=True, figsize=(21, 12), width=0.8
)

plt.title("S&P 500 Breakdown — Sector & Sub-Industry", fontsize=18)
plt.xlabel("Number of Companies", fontsize=14)
plt.ylabel("Sector", fontsize=14)

# Move legend to bottom, spanning multiple columns
