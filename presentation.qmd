...
## Q3 \| Step 1 — Data Acquisition Download OHLCV for `^GSPC(SPX)`, and volatility indices: `^VIX`, `^VVIX`, `^VIX9D` via `yfinance`
...
## Q3 \| Step 2 — Feature Engineering/Standardization


## Q3 \| Step 3 — Train/Test Split 80/20 time based split


## Q3 \| Step 4 — Model Architecture LSTM
```{python}
...
def add_features(
    ...
    ema_span: int = 20,
    rv_window: int = 20,
    vol_variant: str = "rv20"  # "rv20" (default) or "vvix"
) -> Tuple[pd.DataFrame, List[str]]:
    """
    Build 5 high-signal, leak-safe LSTM features:

      1) r1                      : simple daily return
      2) ema_dist_20_pct         : (close / EMA20) - 1
      3) ema_slope_20_pct        : EMA20.pct_change()
      4) rv20 or vvix_over_vix   : realized vol or vol-of-vol spread
      5) vix9d_over_vix_minus1   : term-structure (short vs 30d IV)

    Notes:
    - Uses only info available up to t. No target/shift here (do that in pipeline).
    - Drops warmup NaNs from pct_change/EMA/rolling.
    """
import numpy as np
import pandas as pd
...
def make_features(
    ...
    """
    Builds a compact, stationary feature set for directional LSTM on SPX.

    Continuous features:
      - r1 (always); r5, r21 if add_multi_returns=True
      - ema_dist_20_pct
      - ema_slope_20_pct
      - vix9d_over_vix_minus1
      - rv20  (if use_vvix_spread=False)  OR  vvix_over_vix_minus1 (if True)

    Optional booleans (if add_booleans=True, shifted by 1 to avoid same-bar leakage):
      - aboveEMA20          : close > EMA20  (shifted)
      - ts_inverted         : VIX9D > VIX    (shifted)

    Returns
    -------
    features_df : DataFrame indexed by date (if date_col provided)
    feat_cols   : List[str] of feature column names (continuous + optional booleans)
    """
    out = df.copy()
    ...
    if add_multi_returns:
        extra = [f"r{h}" for h in horizons if h != 1]  # r1 already included
        base_cols += extra
    feat_cols = base_cols + bool_cols

    # Drop warmup NaNs from EMA/rolling/pct_change/shift
    features_df = out.dropna(subset=feat_cols).copy()

    return features_df, feat_cols


def make_windows_arrays(X_scaled, y, lookback: int, horizon: int):
    """(T,F) -> (N,lookback,F) windows with targets at t+h-1."""
    Xw, yw = [], []
    T = len(X_scaled)
...
def split_scale_window(base: pd.DataFrame, feat_cols, H: int):
    """Build X/y, split chronologically, scale on train, then window."""
    # label: future H-day direction
    ...
    scaler = StandardScaler().fit(X_all[:n_train])      # fit on train only
    X_tr_block = scaler.transform(X_all[:n_train])
    X_te_block = scaler.transform(X_all[n_train - LOOKBACK:])  # overlap for first test window
    ...
    # carve out small validation (last 20% of train windows)
...
# code chunk "Model (stacked LSTM classifier)" v1.0

def build_lstm_classifier(timesteps: int, n_features: int) -> tf.keras.Model:
    """2x32 LSTM + dropout + sigmoid head."""
    m = Sequential()
...
# code chunk "Imbalance & threshold utilities" v1.0

def class_weights_from_labels(y: np.ndarray) -> dict:
    """Compute class weights from training labels to counter class imbalance."""
...
def tune_threshold(y_val_true: np.ndarray, y_val_prob: np.ndarray, metric: str = "balacc"):
    """Pick τ on validation to maximize Balanced Accuracy (or F1)."""
...
def score_table(y_true, y_prob, tau):
    """Directional metrics at tuned τ + rank-based AUCs (threshold-free)."""
...
# code chunk "Plots (learning, ROC/PR, confusion@τ)" v1.1
...
## Q3 \| Step 5 — Evaluation Metrics MAE and RMSE for each forecast horizon
